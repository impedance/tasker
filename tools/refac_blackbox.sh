...
echo "[blackbox] structural invariants"

# Invariant: pages are composition-only; they must not orchestrate persistence.
...
# Invariant: domain/features/storage are typed; no explicit-any escape hatches.
# Note: UI remains permissive during MVP; this gate is intentionally scoped.
...
# Invariant: side-effect persistence should not cast move types (T2).
